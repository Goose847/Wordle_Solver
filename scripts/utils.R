...
word_information <- function(word, alphabet_matrix) {
  # I don't think this will yield fundamentally different results.
  # log2 is monotonic and will always be bigger than 1 so prob is still determining factor in ordering.
  log2(1/(1-alphabet_matrix[match(word[1], alphabet), 1])) + 
    log2(1/(1-alphabet_matrix[match(word[2], alphabet), 1])) + 
    log2(1/(1-alphabet_matrix[match(word[3], alphabet), 1])) + 
    log2(1/(1-alphabet_matrix[match(word[4], alphabet), 1])) +
    log2(1/(1-alphabet_matrix[match(word[5], alphabet), 1]))
}
