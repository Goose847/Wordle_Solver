...
correct_guesses <- past_words[which(games_summary$found),1]
incorrect_guesses <- past_words[-which(games_summary$found),1]


guess_history <- map_dfr(game_results, ~tibble(guesses = .x$guesses, found = .x$found))
