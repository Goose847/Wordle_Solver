library(tidyverse)
WORDLE_ANSWER = "" # will pull from an api or possibly find answer history to evaluate
alphabet = c(
  "a",
  "b",
  "c",
...
word_probabilities <-
  apply(valid_words_matrix, MARGIN = 1, word_prob)
...
bayesian_update("pious", c(0,2,0,0,0))
bayesian_update("crane", c(0,0,0,1,0))
bayesian_update("night", c(1,2,0,0,0))
bayesian_update("windy", c(0,2,2,0,1))

valid_words[which.max(word_probabilities),1]
